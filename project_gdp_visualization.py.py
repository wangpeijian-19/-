...
def reconcile_countries_by_name(plot_countries, gdp_countries):  # 返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
    ...
    for i in plot_countries:
        ...
        c = country_name in gdp_countries
        if c == True:
            a[i] = country_name
        if c == False:
            b.add(i)
...
def build_map_dict_by_name(gdpinfo, plot_countries, year):
   ...
   """
    x = reconcile_countries_by_name(plot_countries, gdp_countries)
    dict1 = x[0]
    set1 = x[1]
    set2 = set()
    dict2 = {}
    gdpinfo = pd.read_csv("isp_gdp.csv")
    gdpinfo = np.array(gdpinfo)
    for a in dict1:
        countryname = dict1[a]
...
def test_render_world_map(year):  # 测试函数
    ...
    render_world_map(gdpinfo, pygal_countries, year, "qqisp_gdp_world_name.svg")
